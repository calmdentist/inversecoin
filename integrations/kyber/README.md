...
Upstream **[PR #1692](https://github.com/KyberNetwork/kyberswap-dex-lib/pull/1692)**,
linked to [integration issue #1691](https://github.com/KyberNetwork/kyberswap-dex-lib/issues/1691).
Status: ready for Kyber review; production activation pending.
Adapter commit: `972aa96bcd74524f058f3727d58e4b1ee7fa530b`
(includes upstream `f1eb17be`).
...
`codex/kyber-candidate-reference`. The source repository is now public; anonymous retrieval verified all six
source/configuration hashes against the pinned candidate manifest.
...
```sh
CI=true go test -race -count=1 ./pkg/liquidity-source/uniswap/v4/... ./pkg/msgpack/... ./pkg/pooltypes/...
...
The adapter is ready for implementation review. Production activation requires
a new deployment registered by exact hook address, immutable parameters and
verified bytecode. Then capture a pinned production snapshot, compare both
directions with the deployed Kyber execution adapter, calibrate gas, and have
Kyber deploy its updated quote service. This does not require modifying the
Universal Router.
...
The v4, msgpack and pooltypes suites passed fresh with the race detector against
upstream `f1eb17be` (24 packages with tests). Dispatcher coverage includes 18
sequential buys/sells in each currency ordering, tick/reserve/price/liquidity
updates and quote-failure isolation. The final 30-second purity/determinism fuzz run completed 2,041,256
...
checkout reproduced all fixtures byte-for-byte and passed the reviewed
